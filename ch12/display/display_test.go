...
func TestCircle(t *testing.T) {
	...
	// fmt.Println(s) //..stuck, no output
	// fmt.Println(json.Marshal(s))// fatal error, stack overflow
	...
	// fmt.Println(c) //{42, 0x...}
	// fmt.Println(json.Marshal(c)) //fatal error: stack overflow
...
func TestAd(t *testing.T) {
	...
	// S contains two (promoted) fields named "r", but defined
	// in different packages, so there's no conflict.
	type S struct {
		time.Timer // NB: this struct contains a field named "r" too
		T
	}
	s := S{}
	...
	// The lowercase field name .r is implicitly qualified by this package.
	fmt.Println(reflect.TypeOf(s.r)) // int

	// The string field name "r" matches any package.
	// Two matches are found: s.T.r and s.Timer.r.
	// They annihilate.
	fmt.Println(reflect.TypeOf(s).FieldByName("C")) // true, from Timer.C
	fmt.Println(reflect.TypeOf(s).FieldByName("r")) // false

	Display("s", s)

	Display("nil", nil)
	Display("s", struct{ i interface{} }{i: nil})
	Display("s", struct{ m map[int]bool }{nil})
}
