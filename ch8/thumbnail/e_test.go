...
*/
/*windows
x=1000; while [[ $x > 0 ]]; do { declare x=$(($x - 1)); cp ~/Pictures/fish.jpg ~/Pictures/test/$x.jpg; } done

rm ~/Pictures/test/*.thumb.jpg;
go test -v -bench=BenchmarkMakeThumbnails3 -cpu=4 gopl.io/ch8/thumbnail;
ls ~/Pictures/test/*.thumb.jpg;
...
func init(){
	...
	for i := 0; i < N; i++ {
		...
		files[i] = "/Users/stefaman/Pictures/test/" + strconv.Itoa(i) + ".jpg"
	}
}
...
func BenchmarkMakeThumbnails6(b *testing.B) {
	...
	log.Println(makeThumbnails6(fs))
	}
}
